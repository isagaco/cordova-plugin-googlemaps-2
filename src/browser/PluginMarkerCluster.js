cordova.define("cordova-plugin-googlemaps-2.PluginMarkerCluster", function(require, exports, module) { 
  var utils = require('cordova/utils'),
    ...
    Thread = require('cordova-plugin-googlemaps-2.Thread'),
      ...
      updateClusterIDs.forEach(function(clusterId_markerId) {
        ...
        if (clusterId_markerId.indexOf('-marker_') > -1) {
          ...
          if (isNew) {
            ...
            marker = self.pluginMap.objects[clusterId_markerId];
            // Update the position of the existing cluster marker
            if (markerProperties.lat !== undefined && markerProperties.lng !== undefined) {
              var newPosition = new LatLng(markerProperties.lat, markerProperties.lng);
  ...
  ClusterIconClass.prototype.onRemove = function() {
    ...
    var canvas = self.get('canvas');
    // self.set('map', null);
    const iconMarker = self.get('iconMarker');
    const labelMarker = self.get('labelMarker');
    google.maps.event.clearInstanceListeners(iconMarker);
    google.maps.event.clearInstanceListeners(labelMarker);
